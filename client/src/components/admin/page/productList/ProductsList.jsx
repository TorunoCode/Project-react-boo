...
export default function ProductsList() {
  ...
  { field: 'name', headerName: 'NAME',width: 240, renderCell: (params)=>{
    return(
      ...
      <div className='iq-movie'>
        <a>
          <img src={params.row.image} className='img-border-radius avatar-40 img-fluid' />
        </a>
      </div>
  ...
  { field: 'releaseTime', headerName: 'RELEASE YEAR	', width: 160 ,sortable: false
  , renderCell: (params)=>{
    return(
      <>
     <p> {convert(params.row.releaseTime)}</p>
      </>
    );
  }},
  { field: 'language', headerName: 'LANGUAGE', width: 160 },
  {
    ...
    description: 'This column has a value getter and is not sortable.',width: 160
  },
  {
  ...
  return (
    ...
    <div class="iq-card-header-toolbar d-flex align-items-center"><a class="btn btn-primary" href="/HomeAdmin/newProduct">Add movie</a></div>
       <DataGrid
      rows={data}
